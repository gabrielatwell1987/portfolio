...
const TestedAboutHeroParallax = AboutHeroParallax as unknown as typeof SvelteComponent;
...
describe('AboutHeroParallax Component', () => {
	...
	it('updates layer transform/styles when the page scrolls', async () => {
		...
		const bg = document.querySelector('.layer.background') as HTMLElement | null;
		const middle = document.querySelector('.layer.middle') as HTMLElement | null;
		const fg = document.querySelector('.layer.foreground') as HTMLElement | null;
