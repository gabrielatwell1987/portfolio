<script>
	...
	function toggle() {
		// Use the global transition function for smooth circular reveal
		if (browser && globalThis.transitionTheme) {
			globalThis.transitionTheme();
		} else {
			// Fallback for when ViewTransition isn't loaded yet
	...
	// Initialize theme state based on body class
	$effect(() => {
		if (browser) {
