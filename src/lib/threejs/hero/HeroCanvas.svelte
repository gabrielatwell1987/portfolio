    ...
    $effect(() => {
        ...
        const convertToRGB = (colorValue: string | null): string | null => {
            ...
            try {
                ...
                if (
                    computedColor.startsWith('oklab') ||
                    computedColor === colorValue
                ) {
        ...
        function updateColors() {
            ...
            // detect light mode
            const colorScheme =
                htmlStyles.colorScheme ||
                htmlStyles.getPropertyValue('color-scheme').trim();
            const isLightMode =
                colorScheme === 'light' || colorScheme.startsWith('light');
        ...
        const deferInit = () => {
            ...
            const particlesMesh = new Points(
                particlesGeometry,
                particlesMaterial,
            );
            ...
            const tick = () => {
                ...
                if (mouseX > 0) {
                    particlesMesh.rotation.x =
                        -mouseY * (elapsedTime * 0.0000008);
                    particlesMesh.rotation.y =
                        -mouseX * (elapsedTime * 0.0000008);
                }
