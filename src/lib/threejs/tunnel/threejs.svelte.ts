import {
    ...
    export function setupTunnel(canvas: HTMLCanvasElement) {
        if (!canvas) return;
        ...
        tunnelGeometry.setAttribute(
            'color',
            new BufferAttribute(tunnelColors, 3),
        );
        tunnelMaterial.vertexColors = true;

        let speed = prefersReducedMotion ? 0 : -0.005;
        ...
        function tick() {
            ...
            tunnelMesh.position.z =
                ((tunnelMesh.position.z + tunnelLength / 2) % tunnelLength) -
                tunnelLength / 2;
